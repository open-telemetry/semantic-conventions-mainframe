...
The current model produces **118 unique metric names** emitted across
...
[INFO] Loaded 15 entities, 76 attributes, and 118 metrics (532 upstream stubs available).
[INFO] Active simulation metrics in scope: 118 / 118
...
During implementation it was discovered that five metric names in the
documentation (`mainframe-metrics.md`, `mainframe-zhmc-metrics.md`) differ
from the names actually authored in the model YAML. The simulator emits the
**model names** — the documentation names are aspirational or were superseded:

| Documentation name | Actual model name | Notes |
|---|---|---|
| `mainframe.cpu.thread.utilization` | `mainframe.cpu.thread0.utilization` + `mainframe.cpu.thread1.utilization` | Spec envisioned a single metric discriminated by `mainframe.cpu.thread.id`; model split into two separate metrics |
| `virtualization.partition.cpu.is_capped` | `mainframe.partition.cpu.is_capped` | Moved to `mainframe.partition` namespace during authoring |
| `virtualization.partition.cpu.weight` | `mainframe.partition.cpu.weight.value` | Name and namespace both changed |
| `virtualization.storage_pool.capacity` | `virtualization.storage_pool.capacity.available` | Split into `.available` and `.used` |
| `virtualization.storage_pool.usage` | `virtualization.storage_pool.capacity.used` | — |

Additionally, one metric from `mainframe-zhmc-metrics.md` — `virtualization.partition.cpu.time` —
is listed as `existing_virt_refinement` but has not yet been authored in
`../model/virtualization/metrics_partition.yaml`. It is a known model gap
and not a simulator gap; the simulator will pick it up automatically once it
